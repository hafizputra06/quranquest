...
export default function SurahCard({
  ...
  return (
    <Link href={`/surat/${number}`}>
      <div className={`relative bg-white rounded-xl p-4 border transition-all cursor-pointer hover:shadow-lg ${
        isLastRead 
        ...
        <div className="flex items-start gap-4">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
            isLastRead ? 'bg-emerald-500' : 'bg-emerald-50'
          }`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                revelationType === 'Makkiyah' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'
              }`}>
                {revelationType}
              </span>
